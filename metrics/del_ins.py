...
def del_ins(
    ...
    for idx in tqdm(range(saliency_map.shape[0]), desc="DEL/INS", leave=False):
        ...
        salient_order = np.flip(
            np.argsort(saliency_map[idx].reshape(HW, -1), axis=0), axis=0
        )
        ...
        if arch == "yolox":
            with torch.no_grad():
                for i in range(n_steps + 1):
                    ...
                    temp_score = []
                    torch_start = (
                        torch.from_numpy(start.transpose(2, 0, 1)).unsqueeze(0).float()
                    )
                        ...
                        for b in p_box:
                            ...
                            iou = (
                                torchvision.ops.box_iou(
                                    sample_box[:4].unsqueeze(0),
                                    bbox[idx][:4].unsqueeze(0),
                                )
                                .cpu()
                                .item()
                            )
                            distances = spatial.distance.cosine(
                                sample_score.cpu(), bbox[idx][5:-1].cpu()
                            )
                            weights = math.sqrt(
                                math.exp(-(distances**2) / kernel_width**2)
                            )
                            if target_cls != sample_cls:
                                iou = 0
                                sample_score = torch.tensor(0.0)
                ...
                for i in range(n_steps + 1):
                    ...
                    if len(boxes) is None:
                        ...
                        for box in boxes:
                            ...
                            sample_box = torch.tensor([x_min, y_min, x_max, y_max]).to(
                                device
                            )
                            sample_score = torch.tensor(box[3:]).to(device)
                            iou = (
                                torchvision.ops.box_iou(
                                    sample_box[:4].unsqueeze(0), bbox_box.unsqueeze(0)
                                )
                                .cpu()
                                .item()
                            )
                            distances = spatial.distance.cosine(
                                sample_score.cpu(), bbox_score.cpu()
                            )
                            weights = math.sqrt(
                                math.exp(-(distances**2) / kernel_width**2)
                            )
