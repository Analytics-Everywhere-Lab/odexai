...
class DCLOSE(object):
    ...
    def get_features(self, img):
        ...
        for i in range(self.n_levels):
            ...
            slic_segment = slic(
                img_np, n_segments=num_segments, compactness=10, sigma=1
            )
    ...
    def generate_saliency_map(self, img, box, obj_idx=None):
        """
        box: type(tensor) - shape: [num_boxes, (4 + 1 + num_classes + 1)]
             e.g.: [x_min, y_min, x_max, y_max, p_obj, class_0, class_1, ..., class_n, label]
        img: type(tensor) - shape: [1, 3, H, W]
        obj_idx: optional (int). If provided, only compute saliency map for that index.

        Return saliency maps corresponding to each input bounding box - shape: [num_boxes, H, W].
        """

        # ---------------------------------------------------------
        # 1. If obj_idx is given, reduce 'box' to just that one row.
        #    Otherwise, keep using all bounding boxes.
        # ---------------------------------------------------------
        if obj_idx is not None:
            # box[obj_idx] has shape [7], add dimension so shape becomes [1, 7]
            box = box[[obj_idx], :]

        assert self.arch in [
            "yolox",
            "faster-rcnn",
        ], "This function only supports yolox and faster-rcnn."
        ...
        res = np.zeros(
            (self.n_levels, num_objs, h, w), dtype=np.float32
        )  # [n_levels, num_objs, H, W]
        # Max score for each bounding box
        max_score = np.zeros((num_objs,), dtype=np.float32)  # [num_objs,]
        # Define density map
        ...
        for i in range(self.n_levels):
            for idx_obj in range(res.shape[1]):
                if (
                    res[self.n_levels - 1 - i][idx_obj].max()
                    == res[self.n_levels - 1 - i][idx_obj].min()
                ):
                    res[self.n_levels - 1 - i][idx_obj] = np.zeros_like(
                        res[self.n_levels - 1 - i][idx_obj]
                    )
                else:
                    res[self.n_levels - 1 - i][idx_obj] /= (
                        density_map[self.n_levels - 1 - i].cpu().squeeze().numpy()
                    )
                    res[self.n_levels - 1 - i][idx_obj] = (
                        res[self.n_levels - 1 - i][idx_obj]
                        - res[self.n_levels - 1 - i][idx_obj].min()
