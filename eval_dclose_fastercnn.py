...
for img_path in tqdm(img_paths):
    ...
    with torch.no_grad():
        ...
        boxes = get_prediction_fasterrcnn_only_boxes(prediction, 0.8)
        ...
        # box = boxes[0]
        rs = dclose(inp, boxes)
        visual(img_np, rs, boxes, arch="faster_rcnn")
    ...
    #     continue
    # rgb_saliency_map = cv2.cvtColor(saliency_map, cv2.COLOR_BGR2RGB)
    # resized_saliency_map = resize(rgb_saliency_map, (3, h, w), anti_aliasing=True)
    ...
    # ebpg, pg, count = metric(gt_box, resized_saliency_map[idx_correspond, :, :])
    ...
    #     saliency_map=resized_saliency_map,
    #     arch="fasterrcnn",
    #     mode="del",
    #     step=2000,
    # )
