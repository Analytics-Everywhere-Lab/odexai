...
for img_idx, img_path in tqdm(
    enumerate(img_paths), total=len(img_paths), desc="Img", leave=False
):
    try:
        ...
        for idx in range(len(formatted_boxes)):
            saliency_map = gcame(
                img_tensor.to(device), box=formatted_boxes[idx], obj_idx=idx
            )
        ...
        with torch.no_grad():
            gt_box, idx_correspond = correspond_box(
                formatted_boxes, info_data[file_name]
            )
            ...
            print(
                f"Img {img_idx}: del_auc: {del_auc}, ins_auc: {ins_auc}, ebpg: {ebpg}, pg: {pg}"
            )
